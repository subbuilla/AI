...
#
#Project-1 by Subba Rao Illa (C16280847)
"""
   ...
   # open_set = Stack()
    ...
    while len(open_set)!=0:
        ...
        for child in children:
            if child not in obstacles and child[0] >=0 and child[0]<n_rows and child[1]>=0 and child[1]<n_cols:
            ...
            if child not in obstacles and child[0] >=0 and child[0]<n_rows and child[1]>=0 and child[1]<n_cols:
                if child not in closed_set:
                    if child == goal:
                        ...
                        value = value + costFn(child)
                        if(child not in open_set):
                            open_set.put(child,value)
                            dict[child] = (temp,value)
            ...
            if child not in obstacles and child[0] >=0 and child[0]<n_rows and child[1]>=0 and child[1]<n_cols:
                if child not in closed_set:
                    if child == goal:
                        ...
                        if(child not in open_set):
                            open_set.put(child,value+heuristic(child[0],child[1]))
                            dict[child] = (temp,value)
                        elif(value < dict[child][1]):
                            open_set.remove(child)
                            open_set.put(child,value+heuristic(child[0],child[1]))
