...
class Model(object):
    def __init__(self, learning_rate):
        ...
        #######################################################################
        self.param = {
           'w1': nn.parameter.from_numpy(numpy.random.normal(0, 0.01, [n_features, 256])),
           'w2': nn.parameter.from_numpy(numpy.random.normal(0, 0.01, [256, 256])),
           'w3': nn.parameter.from_numpy(numpy.random.normal(0, 0.01, [256, 256])),
           'w4': nn.parameter.from_numpy(numpy.random.normal(0, 0.01, [256, 256])),
           'w5': nn.parameter.from_numpy(numpy.random.normal(0, 0.01, [256, n_classes])),
           'b1': nn.parameter.zeros(256),
           'b2': nn.parameter.zeros(256),
           'b3': nn.parameter.zeros(256),
           'b4': nn.parameter.zeros(256),
           'b5': nn.parameter.zeros(n_classes)
    ...
    def forward_propagation(self, x):
        ...
        "*** YOUR CODE HERE ***"
        logits = None
        logits=nn.relu(nn.matmul(x,self.param['w1'])+self.param['b1'])
        logits=nn.relu(nn.matmul(logits,self.param['w2'])+self.param['b2'])
        logits=nn.relu(nn.matmul(logits,self.param['w3'])+self.param['b3'])
        logits=nn.relu(nn.matmul(logits,self.param['w4'])+self.param['b4'])
        logits=nn.matmul(logits,self.param['w5'])+self.param['b5']
