...
###############################################################################
    ...
    def max_value(player, board, depth_limit):
        ...
        for cb in child_boards:
            max_reward= value(next_player,cb[1],depth_limit-1)
            if(max_reward[0]>score):
                placement=cb[0]
            score = max(score,max_reward[0])
    ...
    def min_value(player, board, depth_limit):
        ...
        child_boards = get_child_boards(player, board)
        for cb in child_boards:
            min_reward = value(max_player,cb[1],depth_limit-1)
            score=min(score,min_reward[0])
        return score,None

    next_player = board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
    returned_result = value(player,board,depth_limit)
    placement = returned_result[1]
###############################################################################
    return placement
...
###############################################################################
    ...
    def max_value(player, board, depth_limit,alpha,beta):
        ...
        child_boards = get_child_boards(player, board)
        for cb in child_boards:
            max_reward= value(next_player,cb[1],depth_limit-1,alpha,beta)
            if(max_reward[0]>score):
                placement=cb[0]
            score = max(score,max_reward[0])
            if score>=beta:
    ...
    def min_value(player, board, depth_limit,alpha,beta):
        ...
        child_boards = get_child_boards(player, board)
        for cb in child_boards:
            min_reward = value(max_player,cb[1],depth_limit-1,alpha,beta)
            score=min(score,min_reward[0])
            if score<=alpha:
                return score,None
            beta = min(score,beta)
    ...
    next_player = board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
    returned_result = value(player,board,depth_limit,alpha,beta)
    placement = returned_result[1]
...
###############################################################################
    ...
    def min_value(player, board, depth_limit):
        ...
        child_boards = get_child_boards(player, board)
        for cb in child_boards:
            score = score +((1/len(child_boards))*(value(max_player,cb[1],depth_limit-1)[0]) )
        return score,None
