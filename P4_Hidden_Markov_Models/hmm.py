    ...
    def observe(self,
        ...
        for j in range(0,self.n_rows):
            for k in range(0,self.n_cols):
                pe=1.0
                loc=self.get_coordinate(j,k)
                for i in range(0,len(observed_distances)):
                    dist = math.sqrt((loc[0] - landmarks[i][0])**2 + (loc[1] - landmarks[i][1])**2)
                    pe=pe*normal_pdf(dist,App.SENSOR_NOISE,observed_distances[i])
                self.belief[j][k]=self.belief[j][k]*pe
        self.belief = self.normalize(self.belief)
    ...
    def timeUpdate(self):
        ...
        for i in range(0,self.n_rows):
            for j in range(0,self.n_cols):
                trans_model=self.transition_model(i,j)
                sum=0
                current_beliefs=self.belief.copy()
                for val in trans_model:
                    sum=sum+(val[0]*current_beliefs[val[1][0]][val[1][1]])
                current_beliefs[i][j] = sum
                self.belief=current_beliefs.copy()
        self.belief = self.normalize(self.belief)
...
class ParticleFilter(Inference):
    ...
    def observe(self,
        ...
        for i in range (self.NUM_PARTICLES):
            pe=1.0
            loc=self.get_coordinate(self.particles[i][0],self.particles[i][1])
            for j in range(0,len(observed_distances)):
                dist = math.sqrt((loc[0] - landmarks[j][0])**2 + (loc[1] - landmarks[j][1])**2)
                pe=pe*normal_pdf(dist,App.SENSOR_NOISE,observed_distances[j])
            lis.append(pe)
        self.particles=random.choices(self.particles,weights =lis,k=self.NUM_PARTICLES)
        self.updateBelief()
    ...
    def timeUpdate(self):
        ...
        # Please finish the code below
        for i in range (self.NUM_PARTICLES):
            trans_model = self.transition_model(self.particles[i][0],self.particles[i][1])
            rand=random.randint(0,len(trans_model)-1)
            self.particles[i]=trans_model[rand][1]
        self.updateBelief()
